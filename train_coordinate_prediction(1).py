...
def main():
    ...
    max_lead_frames = 15
    default_lead_frames = 15
    sequence_length = input_sequence_length + max_lead_frames  # 输入帧数 + 最大预测步长
    num_sequences = 2000  # 训练序列数量
    num_epochs = 100
    learning_rate = 0.001
    weight_decay = 1e-5
    bullet_speed_mps = 12
    ...
    print(f"   - LSTM层数: {num_lstm_layers}")
    print(f"   - Dropout率: {dropout}")
    print(f"🎯 预测目标: 目标物体坐标 (x, y, w, h)")
    print(f"📊 训练策略: PnP估计预测步长，基于前{input_sequence_length}帧预测目标帧")
    
    try:
        # 1. 创建训练器
        trainer = CoordinatePredictionTrainer()
        ...
        # 3. 创建优化器
        optimizer, scheduler = trainer.create_optimizer(
